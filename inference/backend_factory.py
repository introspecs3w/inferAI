...
class IntBackend:
    "Abstract Interface for Inference Backend"

    def __init__(self, name, config: BackendConfig):
...
class ORTBackend(IntBackend):
    "ONNX Runtime Inference Backend"

    def __init__(self, name, config: BackendConfig, profiler: Profiler):
    ...
    def load(self):
        ...
        model_dir_path = os.path.join(model_dir_path, f"{self.backend_name}_{self.config.provider}_OPT_{self.config.optimization_level}_QUANT_{self.config.quantized}")
        ...
        if(not os.path.exists(model_actual_path)):
            ...
            raise ValueError(f"Model {name} at {model_actual_path} doesn't exist.")
        ...
        self.session = ort.InferenceSession(model_actual_path, sess_options=opts, providers=[provider])
        ...
        # self.tokenizer = tokenClass.from_pretrained("onnx_models")
        # self.tokenizer = tokenClass.from_pretrained(model_dir_path)
        # self.tokenizer = tokenClass.from_pretrained(model_opt_dir_path)
        # self.tokenizer = tokenClass.from_pretrained(model_quant_dir_path)

        self.tokenizer = tokenClass.from_pretrained(tokenizer_actual_path)
        # print(f"tokenizer: {name}")
        # logger.info(f"tokenizer: {name}")
    

        # self.models[name] = session
        # self.model_types[name] = self.config.model_type
    ...
    def run_inference(self, inputs: Any) -> Any:
        ...
        if(self.config.model_type == "text"):
            ...
            logger.info(f"Processing image input for model: {self.config.model_name}")
            ...
            return {
                ...
                "predicted_class_ids": predicted_class_id,
                "image_path": inputs[0]
            }
...
class BackendFactory:
    ...
    _backends: Dict[str, IntBackend] = {}

    @classmethod
    def register_backend(cls, name: str, backend: IntBackend):
        cls._backends[name] = backend

    @classmethod
    def get_backend(cls, name: str, config: BackendConfig, profiler: Profiler) -> IntBackend:
        backend = cls._backends.get(name)
        if backend is None:
            raise ValueError(f"Backend {name} not registered.")
        
        backend_instance = backend(name, config, profiler)
        backend_instance.load()
        return backend_instance



BackendFactory.register_backend("onnx", ORTBackend)
