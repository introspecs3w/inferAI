...
from inference.backend_factory import (
    BackendConfig,
    IntBackend,
    ORTBackend,
    BackendFactory
...
class InferenceManager:
    ...
    def load_models(self):
        ...
        for model in self.config.get("models", []):
            ...
            if model["backend"] != "onnx":
                print(f"Unsupported backend {model['backend']} for model {model['name']}. Skipping.")
                logger.info(f"Unsupported backend {model['backend']} for model {model['name']}. Skipping.")
                continue
            else:   
                backend_config = BackendConfig()
                backend_config.model_name = model["name"]
                backend_config.model_type = model["type"]
                backend_config.provider = model.get("provider", "CPUExecutionProvider")
                backend_config.optimization_level = model.get("optimization_level", ort.GraphOptimizationLevel.ORT_ENABLE_BASIC)
                backend_config.quantized = model.get("quantized", False)

                model_backend = BackendFactory.get_backend(model["backend"], backend_config, self.profiler)
                self.backends[model["name"]] = model_backend
    ...
    def run_inference(self, model_name: str = "default_model", input_text_data: list[str] = None):
        ...
        return self.backends[model_name].run_inference(input_text_data)
